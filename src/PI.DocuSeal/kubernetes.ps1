...
# --------------------------------------------------------------
$DEPLOYMENT="staging/apps/docuseal/deployment.yml"
$APP="docuseal"
...
yq '.docuseal.image = env(TAG)' -i ../../OPS/staging/config/releases.yaml

# #  yq4
# yq '.spec.docuseal.spec.containers[0].image = env(IMAGE)' -i ../../OPS/$DEPLOYMENT 
# yq '.spec.docuseal.spec.containers[0].env[0].name=\"PI_CONTAINER\"' -i ../../OPS/$DEPLOYMENT 
# yq '.spec.docuseal.spec.containers[0].env[0].value = env(TAG)' -i ../../OPS/$DEPLOYMENT 
