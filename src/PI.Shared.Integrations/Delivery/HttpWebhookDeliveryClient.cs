...
/// <summary>Performs the actual HTTP POST of a signed webhook body to a subscriber endpoint.</summary>
...
/// <c>IHttpClientFactory</c> (pooled handlers, no socket exhaustion), signs the body and
...
public sealed class HttpWebhookDeliveryClient : IWebhookDeliveryClient
{
    ...
    public HttpWebhookDeliveryClient(HttpClient httpClient, IWebhookSigner signer, IOptions<DeliveryOptions> options)
    {
    ...
    public async Task<DeliveryResult> DeliverAsync(WebhookDelivery delivery, WebhookEvent webhookEvent, CancellationToken cancellationToken = default)
    {
        ...
        request.Headers.TryAddWithoutValidation(delivery.SignatureHeader ?? "Webhook-Signature", signature);
        request.Headers.TryAddWithoutValidation("Webhook-Id", webhookEvent.Id.ToString("n"));
