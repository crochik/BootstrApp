...
# --------------------------------------------------------------
$DEPLOYMENT="staging/apps/mcp/deployment.yml"
$APP="mcp"
...
yq '.mcp.image = env(TAG)' -i ../../OPS/staging/config/releases.yaml

# #  yq4
# yq '.spec.mcp.spec.containers[0].image = env(IMAGE)' -i ../../OPS/$DEPLOYMENT 
# yq '.spec.mcp.spec.containers[0].env[0].name=\"PI_CONTAINER\"' -i ../../OPS/$DEPLOYMENT 
# yq '.spec.mcp.spec.containers[0].env[0].value = env(TAG)' -i ../../OPS/$DEPLOYMENT 
