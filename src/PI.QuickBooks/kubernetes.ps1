...
# --------------------------------------------------------------
$DEPLOYMENT="staging/apps/quickbooks/deployment.yml"
$APP="quickbooks"
...
yq '.quickbooks.image = env(TAG)' -i ../../OPS/staging/config/releases.yaml

# #  yq4
# yq '.spec.quickbooks.spec.containers[0].image = env(IMAGE)' -i ../../OPS/$DEPLOYMENT 
# yq '.spec.quickbooks.spec.containers[0].env[0].name=\"PI_CONTAINER\"' -i ../../OPS/$DEPLOYMENT 
# yq '.spec.quickbooks.spec.containers[0].env[0].value = env(TAG)' -i ../../OPS/$DEPLOYMENT 
