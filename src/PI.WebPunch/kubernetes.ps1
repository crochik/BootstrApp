...
# --------------------------------------------------------------
$DEPLOYMENT="staging/apps/webpunch/deployment.yml"
$APP="webpunch"
...
yq '.webpunch.image = env(TAG)' -i ../../OPS/staging/config/releases.yaml 

# old way 
# yq '.spec.WebPunch.spec.containers[0].image = env(IMAGE)' -i ../../OPS/$DEPLOYMENT 
# yq '.spec.WebPunch.spec.containers[0].env[0].name=\"PI_CONTAINER\"' -i ../../OPS/$DEPLOYMENT 
# yq '.spec.WebPunch.spec.containers[0].env[0].value = env(TAG)' -i ../../OPS/$DEPLOYMENT 
