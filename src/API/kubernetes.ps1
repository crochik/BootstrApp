...
# --------------------------------------------------------------
$DEPLOYMENT="staging/apps/api/deployment.yml"
$APP="api"
...
yq '.api.image = env(TAG)' -i ../../OPS/staging/config/releases.yaml 
yq '.api2.image = env(TAG)' -i ../../OPS/staging/config/releases.yaml 
